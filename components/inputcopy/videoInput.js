...
export default function VideoInput({
  ...
  const clearImage = (field) => {
    ...
    field.onChange(null);
    if (isStepperVideo) {
      setVideoStepperUrl(null);
    } else {
      setImageUrl(null);
    }
    if (setIsDelete) {
      setIsDelete([...isDelete, videoDeleted]);
    }
  };

  const srcImgUrl = isStepperVideo ? videoStepperUrl : imageUrl;
  return (
    <div>
                ...
                >
                  <div className="flex w-full flex-col items-center justify-center space-y-2 p-3">
                    {srcImgUrl ? (
                      <div className="relative flex h-full w-full justify-center">
                        <video src={srcImgUrl} autoPlay muted loop></video>
                      </div>
                    ) : (
                      <>
                        <svg
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setIsDelete(
                        isDelete?.filter(
                          (itemDelete) => itemDelete !== videoDeleted
                        )
                      );
                    }}
